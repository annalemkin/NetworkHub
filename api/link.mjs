...
//
// Responses: { status: "linked", profileKey } | { status: "needs_linkedin" } | { status: "unmatched" }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const finishLinked = async (profileKey) => {
      ...
      await upsertAccount({ "Clerk user id": userId, ...(email ? { Email: email } : {}), Name: name, "Profile key": profileKey, Status: "Linked", ...(claimed ? { "Claimed LinkedIn": claimed } : {}) });
    ...
    // step 1 — email match, entirely server-side
    const byEmail = await directoryLookup(email);
